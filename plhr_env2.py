...
def simulate_order(order):
    # These probabilities can be adjusted based on your observations of the real market
    probabilities = [0.4, 0.2, 0.1, 0.3]  # probabilities for each outcome
    outcomes = ["matched", "not_matched", "canceled", "partially_matched"]

    outcome = random.choices(outcomes, probabilities)[0]

    if outcome == "matched":
        # Handle the matched order
        return order
    elif outcome == "not_matched":
        # Handle the not matched order
        return 0
    elif outcome == "canceled":
        # Handle the canceled order
        return 0
    elif outcome == "partially_matched":
        # Handle the partially matched order
        partial_fill_percentage = random.uniform(
            0.1, 0.9
        )  # You can also adjust this range
        # Modify the order size based on the partial fill percentage
        return order * partial_fill_percentage
...
if __name__ == "__main__":
    ...
    # # save_rolling_rewards(
    #     file_path="RL/RPPO/RPPO_BayesianRidge/train_monitor_+7_-4.csv",
    #     save_path="RL/RPPO/RPPO_BayesianRidge/rolling_rewards/train_monitor_rolling_+7_-4.csv",
    # )
