...
class Mean120RegressionGreenUp(BaseStrategy):
    def __init__(
        self,
    ...
    def _process_order_for_selection(
        ...
        gambled = self._calculate_green_gambled(side, order.size_matched, price)
        ...
        # Order has not been matched yet so cancel
        elif (
            ...
            matched_tracker[market_id][
                selection_id
            ] = True  # should this be marked as matched as we cancel order???
    ...
    def _process_matched_order(
        ...
        if (order.size_matched >= 10.00 and side == "BACK") or (
            ...
            print("gmarg m8", margin)
            self.metrics["green_margin"] += margin
            self.metrics["amount_gambled"] += gambled

        # We never reach this
        elif not order.size_matched == 0:
            print("MAN WE GOT HERE")
    ...
    def _calculate_green_gambled(
        self, side: str, size_matched: float, price: float
    ) -> float:
        """
        Calculates the green amount gambled based on the side, size matched and price.
    ...
    def _get_adjusted_prices(
        ...
        confidence_number = number + 21 if side == "LAY" else number - 10
